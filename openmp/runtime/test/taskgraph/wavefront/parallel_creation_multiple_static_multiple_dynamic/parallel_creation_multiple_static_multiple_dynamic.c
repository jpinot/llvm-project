...
int test_parallel_creation_multiple_static_multiple_dynamic()
{
    ...
    if (vanilla_result_2 == square[N - 1][N - 1][BS - 1][BS - 1] &&
        ...
        vanilla_result_2 == square2[N - 1][N - 1][BS - 1][BS - 1] &&
        vanilla_result_2 == square3[N - 1][N - 1][BS - 1][BS - 1] &&
        parallelCreation)
        return 1;
...
int main()
{
    ...
    for(i = 0; i < REPETITIONS; i++) {
        if(!test_parallel_creation_multiple_static_multiple_dynamic()) {
            num_failed++;
        }
    }
