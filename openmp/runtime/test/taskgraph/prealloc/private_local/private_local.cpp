...
#pragma omp task private(a_local, b_local, c_local, d_local,          \
                                  e_local, f_local, g_local)
         {
            ...
            if (a_local != -1) ret = 0;
            if (b_local != 5.3) ret = 0;
            if (c_local != &local) ret = 0;
            for (int j = 0; j < 2; j++)
              if (d_local[j] != 4) ret = 0;

            for (int j = 0; j < 4; j++)
              for (int z = 0; z < 4; z++)
                if (e_local[j][z] != 7.2) ret = 0;

            for (int j = 0; j < 2; j++)
              for (int z = 0; z < 2; z++)
                for (int y = 0; y < 2; y++)
                  if (f_local[j][z][y] != 8) ret = 0;
